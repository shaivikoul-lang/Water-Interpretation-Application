...
import {
  CLOUDY_COPY,
  cloudyChecks,
  cloudyDataThroughYear,
  cloudyEvidenceItems,
  dataSourceLabel,
  formatMeasurement,
  turbidityReferenceLabel,
  turbidityTrend,
} from '../../lib/cloudyAppearance'
...
export function CloudyAppearanceResult({
  utilityLabel,
  water,
  onBack,
  onExplore,
  ...
  const regId = useId()
  const checks = cloudyChecks(water)
  const evidence = cloudyEvidenceItems(checks)
  const dataThrough = cloudyDataThroughYear(checks)
  const dataLabel = dataSourceLabel(water)
  const report = getSource('hrw_report_concern')

  return (
    <div className="metal-page cloud-page min-h-svh font-sans">
      <a href="#cloud-main" className="landing-skip-link">
        Skip to main content
      </a>
      <LandingHeader utilityLabel={utilityLabel} />

      <main id="cloud-main" className="metal-main">
        ...
        <div className="metal-hero">
          <div className="metal-title-block">
            ...
            <div>
              <h1 className="metal-title">{CLOUDY_COPY.title}</h1>
              <p className="metal-subtitle">{CLOUDY_COPY.subtitle}</p>
            </div>
          </div>
        </div>

        <div className="metal-grid cloud-grid">
          <aside className="cloud-takeaway metal-takeaway" aria-labelledby="cloud-takeaway-heading">
            <span className="obs-info__badge" aria-hidden>
              <Info className="h-4 w-4" />
            </span>
            <div>
              <h2 id="cloud-takeaway-heading" className="metal-card-h">
                The short answer
              </h2>
              {CLOUDY_COPY.takeaway.map((para) => (
                <p key={para.slice(0, 28)} className="metal-body mt-2">
                  {para}
                </p>
              ))}
            </div>
          ...
          <section className="cloud-meaning metal-card" aria-labelledby="cloud-meaning-heading">
            <h2 id="cloud-meaning-heading" className="metal-card-h">
              <span className="metal-h-icon" aria-hidden>
                <BookOpen className="h-4 w-4" />
              </span>
              What does this usually mean?
            </h2>
            {CLOUDY_COPY.meaning.map((para) => (
              <p key={para.slice(0, 24)} className="metal-body mt-3">
                {para}
              </p>
            ))}
            <p className="metal-body mt-3 text-[13px]">{CLOUDY_COPY.airVsTurbidity}</p>
            <div className="metal-ext-row">
              <SourceLink id="hrw_water_quality_faq">Highlands Ranch Water FAQ</SourceLink>
              <SourceLink id="epa_primary_drinking_water_regulations">EPA: Turbidity</SourceLink>
            </div>
          </section>

          <section className="cloud-next metal-card" aria-labelledby="cloud-next-heading">
            <h2 id="cloud-next-heading" className="metal-card-h">
            ...
            <ol className="mt-4 space-y-3">
              {CLOUDY_COPY.nextSteps.map((step, i) => (
            ...
            <div className="cloud-contact">
              ...
              <a
                className="metal-ext cloud-report"
                href={report.url}
                target="_blank"
                rel="noreferrer"
              >
          ...
          <section className="cloud-related metal-card" aria-labelledby="cloud-related-heading">
            <h2 id="cloud-related-heading" className="metal-card-h">
              <span className="metal-h-icon metal-h-icon--green" aria-hidden>
            ...
            <ul className="mt-3 flex flex-wrap gap-2">
              ...
              <li>
                <button type="button" className="metal-chip" onClick={() => onExplore('hard-water')}>
                  Hardness
                </button>
              </li>
              <li>
          ...
          <section className="cloud-evidence metal-span" aria-labelledby="cloud-evidence-heading">
            <h2 id="cloud-evidence-heading" className="sr-only">
              Evidence
            </h2>
            <button
              type="button"
            ...
            <p className="metal-note">
              ...
              {CLOUDY_COPY.dataNote}
            </p>
            <div id={evidenceId} hidden={!evidenceOpen} className="metal-evidence">
              {evidence.length === 0 ? (
                <p className="metal-body">No turbidity measurements are in this extract.</p>
              ) : (
                      ...
                      <li key={item.id} className="rounded-xl border border-[#d5e3f0] bg-white p-4">
                        ...
                        <dl className="mt-2 grid gap-1 text-[14px] text-[#334E68] sm:grid-cols-2">
                          ...
                          <div>
                            ...
                            <dd>{turbidityReferenceLabel(row)}</dd>
                          </div>
                          <div>
          ...
          <section className="cloud-reg metal-span" aria-labelledby="cloud-reg-heading">
            <h2 id="cloud-reg-heading" className="sr-only">
              Regulatory context
            </h2>
            <button
              type="button"
            ...
            >
              <span className="metal-disclosure__copy">
                ...
                <span>
                  ...
                  <span className="mt-0.5 block text-[13px] font-normal text-[#526D82]">
                    How EPA regulates turbidity as a treatment-technique parameter.
                  </span>
                </span>
              </span>
              <span className="metal-h-icon" aria-hidden>
            ...
            <div id={regId} hidden={!regOpen} className="metal-evidence">
              <p className="metal-body">{CLOUDY_COPY.regulatory}</p>
              <div className="metal-ext-row">
                <SourceLink id="epa_primary_drinking_water_regulations">
          ...
          <section className="cloud-sources metal-span cl-sources" aria-labelledby="cloud-sources-heading">
            <h2 id="cloud-sources-heading" className="metal-card-h">
              <span className="metal-h-icon" aria-hidden>
                <FileText className="h-4 w-4" />
              </span>
              Sources and limitations
            </h2>
            <p className="metal-body mt-3">{CLOUDY_COPY.sourcesIntro}</p>
            <p className="metal-body mt-2 text-[13px]">{CLOUDY_COPY.limitation}</p>
            <div className="cl-sources__split">
              <ol className="cl-sources__list">
                {CLOUDY_COPY.footerCitations.map((citation, index) => {
              ...
              <div className="cl-sources__meta">
                ...
                <p className="cl-sources__note">{CLOUDY_COPY.sourcesNote}</p>
              </div>
            </div>
          </section>
        </div>
